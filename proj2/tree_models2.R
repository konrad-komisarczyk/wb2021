...
d_exp <- read.csv("proj2/data/expired_data.csv") %>% 
  select(-X)

d_icu <- read.csv("proj2/data/icu_data.csv") %>% 
  select(-X)
...
ranger_2 <- ranger(formula = death ~ ., data = exp_train, num.trees = 144, max.depth = 5)
...
# RANDOM FOREST ICU
ranger_5 <- ranger(formula = ICU.or.not ~ ., data = icu_train, num.trees = 359, max.depth = 3)
