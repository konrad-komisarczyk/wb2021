...
We have trained a xgboost model on a combined data from Tongji, St. Antonius and Outcomerea.
...
```{r merge_conf_mat}
render_image("xgboost_merge_confmat.png", suf="")
```
...
```
As expected, first measurements weren't that important to the model and last measurements were better at predicting the outcome.
LDH still seems to be the best predictor, which is also expected as our merged dataset contains Tongji data.
